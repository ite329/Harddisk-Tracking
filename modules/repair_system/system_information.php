...
if (isset($_SESSION["id"]) && isset($_SESSION["pass"])) {
    ...
    if($s_nameserver!=""){
      $stmt = $conn->prepare("SELECT * FROM `system_information` WHERE s_nameserver LIKE ?");
      $search = "%$s_nameserver%";
    ...
    // นับจำนวนแถวทั้งหมดในตาราง system_information
    $sql_count = "SELECT COUNT(*) as total FROM `system_information`";
    ...
    $sql1 = "SELECT * FROM `system_information`  
...
<body>
<div class="container">
  <h3 class="text-center">รายการ ข้อมูลระบบไอทีสารสนเทศ</h3>
  ...
  <form method="post" action="system_information.php" class="form-inline text-center">
    <div class="form-group">
      <label for="">ซื่อระบบ:</label>
      <input type="text" name="s_nameserver" id="s_nameserver" class="form-control" />
    </div>
    <button type="submit" class="btn btn-primary">ค้นหา</button>
    <button type="button" class="btn btn-success" data-toggle="modal" data-target="#add_data_Moda">
            ...
            <table class="table table-bordered table-striped table-hover" >
                <thead>
                    <tr>
                        ...
                        <th style="width: 160px;">ชื่อระบบ</th>
                        <th style="width: 160px;">ทีมพัฒนา</th>
                        <th style="width: 140px;">หัวหน้าทีมพัฒนา</th>
                        <th style="width: 140px;">ผู้พัฒนา/ดูแลหลัก</th>
                        <th style="width: 120px;">เวอร์ชั่นโปรแกรม</th>
                        <th style="width: 120px;">เวอร์ชั่นฐานข้อมูล</th>
                ...
                ?>
                    <tr>
                        <td class="text-center"><?php echo htmlspecialchars($f["system_id"]); ?></td>
                        <td><?php echo htmlspecialchars($f["system_name_th"]); ?></td>
                        <td class="text-center"><?php echo htmlspecialchars($f["development_team"]); ?></td>
                        <td class="text-center"><?php echo htmlspecialchars($f["development_team_leader"]); ?></td>
                        <td class="text-center"><?php echo htmlspecialchars($f["developer_administrator"]); ?>
                        <a href="#" data-toggle="modal" data-target="#edit_da<?php echo $i; ?>"><img src="images/edit.ico" width="10" height="10" alt="แก้ไข" />
                        </a></td>
                        <td class="text-center"><?php echo htmlspecialchars($f["programming_version"]); ?>&nbsp;&nbsp;
                        <a href="#" data-toggle="modal" data-target="#editCaretakerModa<?php echo $i; ?>"><img src="images/edit.ico" width="10" height="10" alt="แก้ไข" />
                        </a></td>
                        <td class="text-center"><?php echo htmlspecialchars($f["database_version"]); ?>&nbsp;&nbsp;
                        <a href="#" data-toggle="modal" data-target="#editlocatio<?php echo $i; ?>"><img src="images/edit.ico" width="10" height="10" alt="แก้ไข" />
                        </a></td>
...
<div id="list<?php echo $i; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="list<?php echo $i; ?>" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-header bg-primary">
        ...
        <h4 class="modal-title">ข้อมูลระบบไอทีสารสนเทศ</h4>
      </div>

      <!-- Body -->
      <div class="modal-body">
        <form class="ajax-form" method="post" action="" novalidate>
                                <label>  ชื่อระบบภาษาอังกฤษ: <?php echo $f["system_name_en"]; ?></label><br>
                                <label>  ชื่อย่อระบบ: <?php echo $f["system_abbreviation"]; ?></label><br>
                                <label>  ชื่อโดเมน: <?php echo $f["domain_server"]; ?></label><br>
                                <label>  Server APP: <?php echo $f["server_app"]; ?></label><br>
                                <label>  ชื่อโฟรเดอร์ App: <?php echo $f["app_path_name"]; ?></label><br>
                                <label>  Server DB: <?php echo $f["server_db"]; ?></label><br>
                                <label>  ชื่อฐานข้อมูล: <?php echo $f["datatbase_name"]; ?></label><br>
                                <label>  ชื่อ User ฐานข้อมูล: <?php echo $f["user_database"]; ?></label><br>
                                </div>
