...
$day2=$_POST["day2"];
if($day1!=""&&$day2!=""){
$sql="SELECT * FROM `report_diy_adapter` WHERE re_diy_ad_day between '$day1'and '$day2'" ;			
$nquery=mysqli_query($conn,$sql);
}
else{
    $sql="SELECT * FROM `report_diy_adapter` ";
    $nquery=mysqli_query($conn,$sql);
}
?>
<!DOCTYPE html>
...
<div align="center">
    <form id="form" name="form" method="post"  action="report_diy.php">
                <h1>รายงาน Adapter ที่ส่งให้สาขาแล้ว</h1>
        <table width="900" height="100" border="1" cellpadding="0" cellspacing="0">
            ...
            while($rs = mysqli_fetch_array($nquery)){
                ...
                $sql2="SELECT * FROM `login` WHERE l_id=$rs[re_diy_ad_l_id] " ;			
                $nq=mysqli_query($conn,$sql2);
                $rs2 = mysqli_fetch_array($nq);
                ?>
            <tr>
                <td align="center"><?=$num ?></td>
                <td align="center"><?=$rs["re_diy_ad_b_id"]?></td>
                <td>&nbsp;&nbsp;<?=$rs["re_diy_ad_name"]?></td>
                <td align="center"><?=$rs["re_diy_ad_num"]?></td>
                <td align="center"><?=$rs["re_diy_ad_day"]?></td>
