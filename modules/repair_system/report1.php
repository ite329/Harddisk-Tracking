...
$b_id=$_POST["b_id"];
$name=$_POST["name"];
$sql = " SELECT * FROM address where b_id= $b_id and a_poin=1 ";
//$sql = " SELECT * FROM address where b_id= $id  ";
...
<table width="1112" height="617" border="0" align="center">
  ...
  <tr height="400">
    <td height="319"><table width="567" height="253" border="0">
      <tr>
		...
		<select align="" style="font: size 28px; font-weight: bold;"  id='distributions' onChange='distributionChange()'>
                ...
                <option value="7">Adapter CCTV</option>
                <option value="13">Adapter Notebook</option>
    ...
    <td valign="bottom"><table width="515" height="265" border="0">
      ...
      <tr>
        <td height="37">สาขา <input type="text" name="b_id" id="b_id" value="<?php echo $f["b_id"]?>" size="1" />&nbsp;&nbsp;ของ&nbsp;&nbsp;
        
        <select name="ba_name" id="ba_name">
                  <option>==&gt;เลือกสาขาย่อย&lt;==</option>
                  <?php
                    $sql1 = " SELECT * FROM address where b_id= $b_id  ";
                   // $sql1 = " SELECT * FROM login where b_id=$id ";
                           $q1 = mysqli_query( $conn, $sql1 );
                           
					while($f1 = mysqli_fetch_assoc( $q1 )){
					?>
                  <option value="<?php echo $f1["b_id"]?>"><?php echo $f1["a_name"]?></option>
                  <?php
					}
