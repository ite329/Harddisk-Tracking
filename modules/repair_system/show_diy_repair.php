...
<div class="container-fluid">
   ...
   <?php  include "menu.php"; ?>
        ...
        <main class="col-md-9 main-content">
            <h2 class="header-title">รายการข้อมูลอุปกรณ์ซ่อม</h2>

            <div class="text-right mb-3">
                <button class="btn btn-warning" onclick="window.location.href='report_diy_adapter.php'">รายงาน adapter ส่งให้สาขา</button>
                <button class="btn btn-warning" onclick="window.location.href='report_diy.php'">รายงาน HDD ส่งให้สาขา</button>
            </div>
            ...
            <div class="row text-center">
                <div class="col-md-6 mb-3">
                    <div class="alert alert-info">
                        <strong>จำนวน HDD ที่เหลืออยู่: </strong><span class="h3"><?php echo $num; ?></span> ลูก
                        <br />
                        <button class="btn btn-primary mt-2" onclick="location.href='frm_diy_add.php'">เพิ่ม HDD</button>
                        <button type="button" class="btn btn-primary mt-2" data-toggle="modal" data-target="#add_data_Modal">เบิก HDD ส่งให้สาขา</button>
                    </div>
                </div>
                <div class="col-md-6 mb-3">
                    <div class="alert alert-info">
                        <strong>จำนวน Adapter ที่เหลืออยู่: </strong><span class="h3"><?php echo $num2["diy_ad_num"]; ?></span> ตัว
                        <br />
                        <button type="button" class="btn btn-warning mt-2" data-toggle="modal" data-target="#add_data_Adapter">เบิก Adapter</button>
                    </div>
                </div>
            </div>
            ...
            <table class="table table-hover table-bordered">
                ...
                <tbody>
                    ...
                    <tr class="<?php echo $rowClass; ?>">
                        ...
                        <?php 
                        if ($rs["d_sn"] != "") {
                            echo htmlspecialchars($rs["d_sn"]);
                        } else {
                            echo '<button class="btn btn-sm btn-warning" data-toggle="modal" data-target="#add_data_sn' . $i . '">SN_HDD</button>';
                        }
                        ?>
                        </td>
                    ...
                    <!-- Modal เพิ่ม SN HDD -->
                    <div id="add_data_sn<?php echo $i; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalLabel_sn<?php echo $i; ?>">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header bg-primary">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h4 class="modal-title">เบิก HDD</h4>
                          </div>
                          <div class="modal-body">
                            <form class="ajax-form" action="edit_diy.php" method="post" novalidate>
                              <div class="form-group">
                                <label>กรอก SN_HDD ที่จะส่ง  คนแพ็ค: <?php echo htmlspecialchars($rs6['l_name']); ?></label>
                                <input type="hidden" name="re_diy_id" value="<?php echo $rs['re_diy_id']; ?>">
                                <input type="hidden" name="id" value="<?php echo $id; ?>">
                                <input name="d_sn" type="text" id="d_sn" class="form-control" required autofocus>
                              </div>
...
<!-- Modal เบิก Adapter -->
<div id="add_data_Adapter" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalLabel_Adapter">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">สาขาที่จะเบิก</h4>
      </div>
      <div class="modal-body">
        <form class="ajax-form" action="save_diy_adapter.php" method="post" novalidate>
          <div class="form-group">
            <label>พนักงานผู้เบิกของ <?php echo htmlspecialchars($rs6['l_name']); ?></label>
            <input type="hidden" name="l_id" value="<?php echo $id; ?>">
            <br>
            <label style="color:#FF0000" style="font-size:20px;">ให้ลงไปแพ็คส่งให้สาขาด้วย</label>
          </div>
          <div class="form-group">
            <label>รหัสสาขาใหญ่</label>
            <input name="r_cho" type="text" id="r_cho" onkeypress="check_key_number(event);" value="000" class="form-control" required>
          </div>
          <div class="form-group">
            <label for="r_cho_ru">ชื่อสาขา</label>
            <select name="r_cho_ru" disabled id="r_cho_ru" class="form-control" required>
              <option value="">==>กรุณาเลือกสาขา<==</option>
            </select>
          </div>
          <div class="form-group">
            <label>จำนวนที่เบิก</label>
            <input name="re_diy_ad_num" type="text" id="re_diy_ad_num" onkeypress="check_key_number(event);" value="00" class="form-control" required>
          </div>
          <div class="form-group">
            <label>วันที่บันทึกข้อมูล</label>
            <input name="re_diy_day" type="text" id="dateInput2" value="<?php echo date('Y-m-d'); ?>" class="form-control" required>
          </div>
          <button type="submit" class="btn btn-success btn-block">บันทึก</button>
        </form>
      </div>
...
<div id="add_data_Modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modalLabel_diy">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form class="ajax-form" action="save_diy.php" method="post" novalidate>
          <div class="form-group">
            <label>พนักงานผู้เบิกของ <?php echo htmlspecialchars($rs6['l_name']); ?></label>
            <input type="hidden" name="l_id" value="<?php echo $id; ?>">
          </div>
 ...
 <!--แก้ไขตำแหน่งที่ตั้ง-->      
        <script type="text/javascript">
    // ฟังก์ชันเปิด Popup สำหรับข้อมูลเพิ่มเติม
    ...
    const popupWindow = window.open("", "popupWindow", `width=${width},height=${height},top=${top},left=${left}`);
    popupWindow.document.write(`
        <html lang="th">
            ...
            <body>
                <h3 align="center">รหัสทรัพย์สิน: ${data}</h3>
                <h4 align="center">แก้ไขตำแหน่งคอมพิวเตอร์</h4>
                <form action="itedit_position.php" method="GET">
                    <label for="cho1">เลือกข้อมูล: </label>
                    <select name="cho1" id="cho1" class="form-control">
                        <option value="0">ด้านซ้าย</option>
                        <option value="1">ด้านขวา</option>
                        <option value="2">ด้านกลาง</option>
                    </select>
