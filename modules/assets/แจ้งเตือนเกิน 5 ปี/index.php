...
if (!function_exists('assetIsPrinterItem')) {
    function assetIsPrinterItem($value): bool
    {
        ...
        $normalize = static function (string $text): string {
            ...
            // ตัดช่องว่าง ขีด จุด วงเล็บ และอักขระคั่น เพื่อรองรับชื่อรุ่นที่บันทึกต่างรูปแบบกัน
            $text = preg_replace('/[^\p{L}\p{N}]+/u', '', $text);
            return trim((string)$text);
        };

        $printerNames = [
            'PRINTER HP รุ่น LASERJET ENTERPRISE MFP M430fdn',
            'HP LaserJet Enterprise MFP M430 series',
            'HP LaserJet Pro M402dn',
            'HP LaserJet Pro M404dn',
            'เครื่องถ่ายเอกสารเล็ก HP M402 dn',
            'PRINTER HP รุ่น LASER JET PRO 400 MFP M426fdn',
            'HP LaserJet Pro M426fdn MFP',
            'PRINTER HP รุ่น LASERJET MFP M428fdn',
            'HP LaserJet Pro MFP M428fdn',
            'PRINTER BROTHER รุ่น DCP-L5600DN',
            'PRINTER BROTHER รุ่น MFC-L5915DW',
            'PRINTER BROTHER รุ่น MFC-L5900DW',
            'เครื่องปริ้น',
        ];

        $normalizedItem = $normalize($item);
        foreach ($printerNames as $printerName) {
            $normalizedPrinterName = $normalize($printerName);
            if ($normalizedPrinterName !== '' && ($normalizedItem === $normalizedPrinterName || strpos($normalizedItem, $normalizedPrinterName) !== false)) {
                return true;
            }
        }

        // สำรองการตรวจจากรหัสรุ่น เพื่อรองรับข้อความนำหน้า/ต่อท้ายหรือชื่อที่สะกดต่างกันในฐานข้อมูล
        $printerModelTokens = [
            'm430fdn', 'm430series', 'm402dn', 'm404dn', 'm426fdn', 'm428fdn',
            'dcpl5600dn', 'mfcl5915dw', 'mfcl5900dw'
        ];
        foreach ($printerModelTokens as $modelToken) {
            if (strpos($normalizedItem, $modelToken) !== false) {
                return true;
            }
        }

        return strpos($normalizedItem, $normalize('เครื่องปริ้น')) !== false;
    }
}

if (!function_exists('assetBuildPrinterAgeAlerts')) {
    function assetBuildPrinterAgeAlerts(array $rows): array
    {
        $alerts = [];
        foreach ($rows as $row) {
            $item = assetPickValue($row, 'asset_item');
            $ageDate = assetPickValue($row, 'age_base_date');
            if ($ageDate === null) $ageDate = assetPickValue($row, 'received_date');
            if (!assetIsPrinterItem($item) || !assetAgeExceedsYears($ageDate, 5)) continue;

            $assetCode = assetFormatText(assetPickValue($row, 'asset_new_code'));
            $key = $assetCode !== '-' ? $assetCode : md5(json_encode($row, JSON_UNESCAPED_UNICODE));
            $alerts[$key] = [
                'asset_code' => $assetCode,
                'asset_item' => assetFormatText($item),
                'branch_name' => assetDisplayBranchName(assetPickValue($row, 'branch_name')),
                'received_date' => assetFormatDateThai($ageDate),
                'age' => assetCalculateAgeFromDate($ageDate),
            ];
        }
...
$printerAgeAlerts = [];
if ($assetError === '' && $assetKeyword !== '' && !empty($assetRows)) {
    // ตรวจเฉพาะคอลัมน์รายการทรัพย์สินของผลค้นหาจากรหัสทรัพย์สิน
    // ต้องเป็นเครื่องปริ้นตามรายชื่อที่กำหนด และมีอายุเกิน 5 ปี จึงแสดง Popup
    $printerAgeAlerts = assetBuildPrinterAgeAlerts($assetRows);
}
                    ...
                    <tbody>
                        <?php foreach ($assetRows as $asset): ?>
                            ...
                            <tr>
                                ...
                                <td class="branch-cell asset-search-value-red">
                                    <?php
                                        $branchLink = '';
                            ...
                            <tbody>
                                <?php foreach ($branchAssetRows as $rowIndex => $branchAsset): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-wrap-col"><?php echo assetE(assetDisplayBranchName($branchAsset['as_name'] ?? null)); ?></td>
                    ...
                    <div class="asset-full-detail-grid">
                        <?php foreach ($orderedDetailFields as $detailField): ?>
                            ...
                            <div class="asset-full-detail-item<?php echo $detailField['type'] === 'age' ? ' primary' : ''; ?>">
                                ...
                                <div class="value<?php echo in_array($columnName, ['as_name', 'as_code_new', 'as_list'], true) ? ' asset-highlight-red' : ''; ?>">
...
<?php if (!empty($printerAgeAlerts)): ?>
<div class="modal fade" id="printerAgeWarningModal" tabindex="-1" aria-labelledby="printerAgeWarningModalLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-danger text-white">
                <div>
                    <h5 class="modal-title fw-bold" id="printerAgeWarningModalLabel"><i class="bi bi-exclamation-triangle-fill me-2"></i>แจ้งเตือนอายุการใช้งานเครื่องปริ้น</h5>
                    <div class="small opacity-75">พบเครื่องปริ้นที่มีอายุการใช้งานเกิน 5 ปี</div>
                </div>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="ปิด"></button>
            </div>
            <div class="modal-body bg-light">
                <div class="alert alert-danger py-2">กรุณาตรวจสอบสภาพเครื่องและพิจารณาแผนซ่อมบำรุงหรือทดแทน</div>
                <div class="table-responsive rounded border bg-white">
                    <table class="table table-sm table-bordered align-middle mb-0">
                        <thead class="table-light"><tr><th class="text-center">#</th><th>รหัสทรัพย์สิน</th><th>รายการเครื่องปริ้น</th><th>สาขา</th><th>วันที่รับเข้า</th><th class="text-center">อายุใช้งาน</th></tr></thead>
                        <tbody>
                        <?php foreach ($printerAgeAlerts as $warningIndex => $printerWarning): ?>
                            <tr>
                                <td class="text-center"><?php echo number_format($warningIndex + 1); ?></td>
                                <td class="fw-bold text-danger"><?php echo assetE($printerWarning['asset_code']); ?></td>
                                <td><?php echo assetE($printerWarning['asset_item']); ?></td>
                                <td><?php echo assetE($printerWarning['branch_name']); ?></td>
                                <td><?php echo assetE($printerWarning['received_date']); ?></td>
                                <td class="text-center"><span class="badge text-bg-danger"><?php echo assetE($printerWarning['age']); ?></span></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <div class="modal-footer"><button type="button" class="btn btn-danger px-4" data-bs-dismiss="modal">รับทราบ</button></div>
        </div>
    </div>
</div>
<?php endif; ?>
                ...
                <div class="table-responsive asset-detail-popup-table-wrap">
                    <table class="table table-bordered table-hover asset-detail-popup-table">
                        <tbody>
                            ...
                            <tr><th>ชื่อสาขา</th><td><div class="asset-detail-popup-value-wrap"><span class="asset-detail-popup-value-text asset-highlight-red" data-popup-field="branch-name">-</span></div></td></tr>
...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var printerWarningModal = document.getElementById('printerAgeWarningModal');
        if (printerWarningModal && window.bootstrap && bootstrap.Modal) {
            window.setTimeout(function () {
                bootstrap.Modal.getOrCreateInstance(printerWarningModal).show();
            }, 0);
        }
    });
    <?php endif; ?>
