...
try {
    ...
    if ($hasBranchCodeColumn) {
        $sql = "SELECT dw.request_no, dw.main_branch_code, dw.branch_code, dw.branch_name,
                       ...
                       GROUP_CONCAT(dw.drum_code ORDER BY dw.drum_code SEPARATOR ',') AS drum_items,
...
.drum-add-drum-item:has(.drum-checkbox:checked) .drum-add-branch-result {
    margin-top: 8px;
}
...
.drum-edit-drum-item:has(.edit-drum-checkbox:checked) .drum-delete-modal .modal-dialog {
    max-width: 520px;
}
            ...
            <tr data-drum-request-row="<?php echo e($row['request_no']); ?>">
              ...
              <td class="text-center">
                <div class="drum-action-buttons">
                  <?php if ($canEditDrum): ?><button type="button" class="btn btn-outline-primary js-drum-edit" title="แก้ไข" aria-label="แก้ไข" data-bs-toggle="modal" data-bs-target="#drumEditModal" data-request-no="<?php echo e($row['request_no']); ?>" data-main-branch-code="<?php echo e($row['main_branch_code']); ?>" data-branch-code="<?php echo e($row['branch_code'] ?: ''); ?>" data-branch-name="<?php echo e($row['branch_name']); ?>" data-drum-codes="<?php echo e($row['drum_codes']); ?>" data-drum-items="<?php echo e($row['drum_items'] ?? ''); ?>" data-problem-no="<?php echo e($row['problem_no'] ?? ''); ?>" data-remark="<?php echo e($row['remark'] ?? ''); ?>" data-recorded-by="<?php echo e($row['recorded_by']); ?>" data-created-at="<?php echo e(date('Y-m-d\TH:i', strtotime($row['created_at']))); ?>"><svg class="action-icon" width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10A.5.5 0 0 1 5.5 14H2a.5.5 0 0 1-.5-.5V10a.5.5 0 0 1 .146-.354zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zM12.793 5.5 10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zM3.5 10.207 2.5 11.207V13h1.793l1-1H5.5v-.5H5a.5.5 0 0 1-.5-.5v-.5H4a.5.5 0 0 1-.5-.5z"/></svg></button><?php endif; ?>
                  ...
                  <?php if ($canDeleteDrum): ?><button type="button" class="btn btn-outline-danger js-drum-delete" title="ลบ" aria-label="ลบ" data-bs-toggle="modal" data-bs-target="#drumDeleteModal" data-request-no="<?php echo e($row['request_no']); ?>"><svg class="action-icon" width="14" height="14" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 .5a.5.5 0 0 0-1 0v6a.5.5 0 0 0 1 0z"/><path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1 0-2H5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1h2.5a1 1 0 0 1 1 1M4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM6.5 2a.5.5 0 0 0-.5.5V3h4v-.5a.5.5 0 0 0-.5-.5z"/></svg></button><?php endif; ?>
                </div>
              </td>
              <?php endif; ?>
            </tr>
        ...
        <div class="table-responsive border rounded-3 bg-white"><table class="table table-bordered drum-edit-table align-middle">
          <tbody>
            ...
            <tr><th>รายการ Drum <span class="text-danger">*</span></th><td><div class="drum-edit-drums">
              <label class="drum-edit-drum-item"><input class="form-check-input me-2 edit-drum-checkbox" type="checkbox" name="drum_codes[]" value="Drum-DR-3455"><span>Drum-DR-3455</span></label>
              <label class="drum-edit-drum-item"><input class="form-check-input me-2 edit-drum-checkbox" type="checkbox" name="drum_codes[]" value="Drum-DR-3608"><span>Drum-DR-3608</span></label>
            </div></td></tr>
              ...
              <tr>
                ...
                <td>
                  <div class="drum-add-drums">
                    ...
                    <label class="drum-add-drum-item"><input class="visually-hidden drum-checkbox" type="checkbox" name="drum_codes[]" value="<?php echo e($code); ?>" id="<?php echo e(str_replace('-','_',$code)); ?>"><span><strong><?php echo e($code); ?></strong><small><?php echo e($desc); ?></small></span></label>
                    <?php endforeach; ?>
                  </div>
                </td>
              </tr>
...
async function checkDuplicateHistory() {
    ...
    try {
        ...
        const response = await fetch(
            'check_duplicate.php?' + params.toString() + '&_=' + Date.now(),
            {
                headers: {
        ...
        try {
            ...
            console.error('[Drum duplicate check] Non-JSON response:', responseText.slice(0, 500));
            throw new Error('ไม่สามารถตรวจสอบรายการซ้ำได้ในขณะนี้');
        }
        ...
        duplicateMatches = [];
        hideDuplicateWarning();
        console.error('[Drum duplicate check]', error);
        // API/JSON errors are system errors, not duplicate records. Do not show the duplicate warning box.
        // Keep save disabled until duplicate verification succeeds to avoid bypassing the check silently.
        save.disabled = true;
    }
}
 ...
 function resetForm(){form.reset();code.value='';clearSelected();document.querySelectorAll('.drum-checkbox').forEach(item=>item.checked=false);hideDuplicateWarning();validate();}
 ...
 code.addEventListener('input',function(){
   ...
   clearSelected();
   window.clearTimeout(autoLookupTimer);
   if(/^\d{3}$/.test(this.value)){
     autoLookupTimer=window.setTimeout(function(){lookup();},250);
   }
 });
 ...
 document.querySelectorAll('.drum-checkbox').forEach(function(x){x.addEventListener('change',checkDuplicateHistory);});
 ...
 form.addEventListener('submit',e=>{if(!branchCodeInput.value||!nameInput.value||!document.querySelector('.drum-checkbox:checked')||!String(problemNo.value||'').trim()){e.preventDefault();if(!String(problemNo.value||'').trim()){problemNo.focus();alert('กรุณากรอกเลขที่ปัญหา');}else{alert('กรุณาค้นหา เลือกสาขาปลายทาง และเลือก Drum อย่างน้อย 1 รายการ');}return;}if(duplicateMatches.length&&String(repeatReason.value||'').trim().length<5){e.preventDefault();repeatReason.focus();alert('พบรายการเบิกหรือประวัติการจัดส่งซ้ำ กรุณาระบุเหตุผลในการส่งซ้ำอย่างน้อย 5 ตัวอักษร');return;}});
 ...
 document.querySelectorAll('.js-drum-edit').forEach(function(button){button.addEventListener('click',function(){
   ...
   const selectedDrumCodes=new Set();
   const drumItems=(button.dataset.drumItems||'').split(',').map(v=>v.trim()).filter(Boolean);
   if(drumItems.length){drumItems.forEach(function(item){const parts=item.split('|');const drumCode=String(parts[0]||'').trim();if(drumCode)selectedDrumCodes.add(drumCode);});}
   else{(button.dataset.drumCodes||'').split(',').map(v=>v.trim().split(' x')[0]).filter(Boolean).forEach(function(drumCode){selectedDrumCodes.add(drumCode);});}
   document.querySelectorAll('.edit-drum-checkbox').forEach(function(cb){cb.checked=selectedDrumCodes.has(cb.value);});
