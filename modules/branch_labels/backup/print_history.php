...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['manage_action'])) {
    try {
        ...
        if ($manageAction === 'edit') {
            ...
            if (!in_array($source, ['direct_branch', 'main_branch_group'], true)) {
...
try {
    ...
    if (($_GET['export'] ?? '') === 'csv') {
        $exportStmt = $pdo->prepare("SELECT printed_at, printed_by_name, printed_by_employee_code, main_branch_code, branch_code, branch_name, asset_name, shipping_address, print_orientation, print_source, printed_ip
        ...
        fputcsv($out, ['วันที่พิมพ์', 'ผู้พิมพ์', 'รหัสพนักงาน', 'รหัสสาขาใหญ่', 'Cost Center', 'สาขาปลายทาง', 'รายการจัดส่ง', 'ที่อยู่จัดส่ง', 'รูปแบบกระดาษ', 'แหล่งการพิมพ์', 'IP Address']);
        while ($row = $exportStmt->fetch(PDO::FETCH_ASSOC)) {
            fputcsv($out, [
                $row['printed_at'], $row['printed_by_name'], $row['printed_by_employee_code'],
                $row['main_branch_code'], $row['branch_code'], $row['branch_name'],
                $row['asset_name'], $row['shipping_address'], $row['print_orientation'],
                $row['print_source'], $row['printed_ip'],
            ]);
        }
        fclose($out);
        exit;
    ...
    /* แสดงเฉพาะรายการล่าสุดเมื่อข้อมูลซ้ำกัน โดยไม่ลบข้อมูลเดิมออกจากฐานข้อมูล */
    $latestHistorySql = "SELECT MAX(id) AS latest_id
        FROM `harddisk_db`.`branch_label_print_history`
        GROUP BY
            COALESCE(main_branch_code, ''),
            COALESCE(branch_code, ''),
            COALESCE(branch_name, ''),
            COALESCE(shipping_address, ''),
            COALESCE(asset_name, ''),
            COALESCE(print_orientation, ''),
            COALESCE(print_source, '')";

    $countStmt = $pdo->prepare("SELECT COUNT(*)
        FROM `harddisk_db`.`branch_label_print_history` history
        INNER JOIN ({$latestHistorySql}) latest_history
            ON latest_history.latest_id = history.id
        {$whereSql}");
    ...
    $stmt = $pdo->prepare("SELECT history.id, history.main_branch_code, history.branch_code, history.branch_name, history.shipping_address, history.asset_name,
...
<nav class="branch-label-module-menu" aria-label="เมนูระบบค้นหาและพิมพ์ที่อยู่สาขา">
    ...
    <div class="row g-2 mb-3">
        <?php foreach ([['พิมพ์ทั้งหมด',$summary['all']],['พิมพ์วันนี้',$summary['today']],['7 วันที่ผ่านมา',$summary['week']],['30 วันที่ผ่านมา',$summary['month']],['สาขาไม่ซ้ำ',$summary['branches']]] as $kpi): ?>
        <div class="col-6 col-md"><div class="card print-history-kpi h-100"><div class="card-body"><div class="print-history-kpi-label"><?php echo phE($kpi[0]); ?></div><div class="print-history-kpi-value"><?php echo number_format((int)$kpi[1]); ?></div></div></div></div>
        <?php endforeach; ?>
    </div>
    ...
    <div class="card print-history-filter mb-3"><div class="card-body p-3">
        ...
        <form method="get" class="print-history-filter-grid" autocomplete="off">
            ...
            <div><label class="form-label small fw-bold">แหล่งการพิมพ์</label><select name="print_source" class="form-select"><option value="">ทั้งหมด</option><option value="direct_branch" <?php echo $printSource === 'direct_branch' ? 'selected' : ''; ?>>ระบบเดิม</option><option value="main_branch_group" <?php echo $printSource === 'main_branch_group' ? 'selected' : ''; ?>>สาขาในสังกัด</option></select></div>
    ...
    <div class="card print-history-table-card">
        ...
        <div class="table-responsive"><table class="table table-hover table-bordered align-middle mb-0 print-history-table"><thead><tr><th>ลำดับ</th><th>รหัสสาขาใหญ่</th><th>สาขาปลายทาง</th><th>รายการจัดส่ง</th><th>ที่อยู่จัดส่ง</th><th>วันที่พิมพ์</th><th>ระบบ</th><?php if ($canManagePrintHistory): ?><th>จัดการ</th><?php endif; ?></tr></thead><tbody>
        <?php if (!$rows): ?><tr><td colspan="<?php echo $canManagePrintHistory ? 8 : 7; ?>" class="text-center text-muted py-5">ยังไม่มีประวัติการพิมพ์ตามเงื่อนไขที่เลือก</td></tr><?php else: foreach ($rows as $index => $row): ?>
            <tr><td class="text-center"><?php echo number_format((($page-1)*$perPage)+$index+1); ?></td><td class="fw-bold text-primary text-center"><?php echo phE($row['main_branch_code']); ?></td><td><?php echo phE($row['branch_name']); ?></td><td><?php echo phE($row['asset_name'] ?: '-'); ?></td><td><div class="print-history-address" title="<?php echo phE($row['shipping_address']); ?>"><?php echo phE($row['shipping_address'] ?: '-'); ?></div></td><td class="text-nowrap text-center"><?php echo phE(date('d/m/Y', strtotime((string)$row['printed_at']))); ?></td><td class="text-center"><span class="badge <?php echo $row['print_source'] === 'main_branch_group' ? 'text-bg-info' : 'text-bg-secondary'; ?> print-history-source"><?php echo $row['print_source'] === 'main_branch_group' ? 'สาขาในสังกัด' : 'ระบบเดิม'; ?></span></td><?php if ($canManagePrintHistory): ?><td><div class="print-history-actions"><button type="button" class="btn btn-warning btn-sm js-edit-print-history" data-bs-toggle="modal" data-bs-target="#editPrintHistoryModal" data-id="<?php echo (int)$row['id']; ?>" data-main-branch-code="<?php echo phE($row['main_branch_code']); ?>" data-branch-code="<?php echo phE($row['branch_code']); ?>" data-branch-name="<?php echo phE($row['branch_name']); ?>" data-shipping-address="<?php echo phE($row['shipping_address']); ?>" data-asset-name="<?php echo phE($row['asset_name']); ?>" data-print-orientation="<?php echo phE($row['print_orientation']); ?>" data-print-source="<?php echo phE($row['print_source']); ?>">แก้ไข</button><button type="button" class="btn btn-outline-danger btn-sm js-delete-print-history" data-bs-toggle="modal" data-bs-target="#deletePrintHistoryModal" data-id="<?php echo (int)$row['id']; ?>" data-branch-name="<?php echo phE($row['branch_name']); ?>" data-printed-at="<?php echo phE(date('d/m/Y', strtotime((string)$row['printed_at']))); ?>">ลบ</button></div></td><?php endif; ?></tr>
        ...
        <form method="post" class="modal-content" autocomplete="off">
            ...
            <div class="modal-body">
                ...
                <div class="print-history-edit-grid">
                    ...
                    <div class="full"><label class="form-label">แหล่งการพิมพ์</label><select name="print_source" id="edit_print_source" class="form-select"><option value="direct_branch">ปริ้นที่อยู่สาขาย่อย</option><option value="main_branch_group">ปริ้นที่อยู่สาขาใหญ่</option></select></div>
                </div>
            </div>
